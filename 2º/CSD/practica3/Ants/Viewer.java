...
public class Viewer extends Panel {
    ...
    public Viewer(int cols, int ants, int movs, String msg) {
        ...
        states.setFont(new Font(Font.MONOSPACED,Font.PLAIN,14));
    ...
    public void paint(Graphics g) {
       g.setFont(new Font("Helvetica",Font.BOLD,16));g.setColor(new Color(54,26,224)); g.drawString(msg+"   "+retries+" retries",10,20);
       g.setColor(Color.GRAY); g.setFont(plain);
        drawGrid(g); drawXrule(g); drawYrule(g);g.setColor(Color.BLACK);
        if (idx<0) return;
