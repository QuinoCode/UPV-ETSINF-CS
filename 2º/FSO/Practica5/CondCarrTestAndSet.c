...
*/
//Esto por algún motivo es código de ensamblador de intel, y fuciona en C
...
int main (int argc, char *argv[]) {
  ...
  // PARA CREAR UN HILO HAY QUE SEGUIR LA SIGUIENTE ESTRUCTURA:
  //int pthread_create(pthread_t *thread, const pthread_attr_t *attr, void *(*start_routine)(void *), void *arg);
  // donde *thread es un puntero al identificador del hilo, *attr es un puntero a los atributos del hilo (por defecto: joinable, y con default (non real-time)scheduling policy)
  //*(*start_routine) es el puntero que apunta a la función que queremos ejecutar y *arg es un puntero al argumento que se le pasaría a la fución de start_routine, si necesitara uno.
  // Devolverá 0 si no hay problema en la creación y un número != 0 si lo hay
